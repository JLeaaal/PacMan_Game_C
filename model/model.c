...
const char *model_skin_name(int index) {
    static const char *names[SKIN_COUNT] = {
        "Pac-Man",
        "Ms. Pac-Man"
    };
    if (index < 0 || index >= SKIN_COUNT) return names[0];
    return names[index];
}

const char *model_skin_color(int index) {
    /* Always render Pac-Man as yellow; map color is handled in view */
    return CLR_PACMAN;
}

static int model_compute_frame_ms(GameModel *m) {
    int base = FRAME_MS - (m->level - 1) * 4;
    int eaten = m->total_pellets - m->pellets_left;
    base -= eaten / 18;
    if (base < 24) base = 24;
    return base;
}

int model_load_scores(GameModel *m) {
    ...
    int count = 0;
    while (count < 10 && fscanf(fp, "%d %15s\n", &score, name) == 2) {
...
int model_save_scores(GameModel *m) {
    int scores[20];
    char names[20][16];
    int count = 0;
...
int model_score_qualifies(GameModel *m, int score) {
    if (bst_count(m->score_tree) < 10) return 1;
    BSTNode *mn = bst_min(m->score_tree);
    return mn && score > mn->score;
}
...
static void model_apply_map_variant(GameModel *m, int variant) {
    if (variant == 1) {
...
static int model_is_special_cell(int x, int y, int variant) {
    if (x <= 1 || x >= MAP_W - 2 || y <= 1 || y >= MAP_H - 2) return 1;
    if ((x >= 12 && x <= 15) && (y >= 11 && y <= 15)) return 1;
    if ((x == 14 && y == 23) || (x == 14 && y == 11)) return 1;
    if (y == TUNNEL_ROW && (x == 0 || x == MAP_W - 1)) return 1;

    if (variant == 1) {
        if ((x >= 10 && x <= 17) && (y >= 10 && y <= 18)) return 1;
    } else if (variant == 2) {
        if (((x == 7 || x == 13 || x == 14 || x == 20) && y >= 2 && y <= MAP_H - 3) ||
            ((y == 8 || y == 15 || y == 22) && x >= 2 && x <= MAP_W - 3)) {
            return 1;
        }
    }
    return 0;
...
static void model_apply_randomized_layout(GameModel *m, int variant) {
    static unsigned int map_seed = 0;
    map_seed += 31u + (unsigned int)m->level * 17u;
    srand(map_seed);

    int mutations = 12 + (m->level % 4);
    for (int i = 0; i < mutations; i++) {
        int x = 2 + rand() % (MAP_W - 4);
        int y = 2 + rand() % (MAP_H - 4);
        if (model_is_special_cell(x, y, variant)) continue;

        int tile = m->grid[y][x];
        if (tile == TILE_WALL) {
            if (rand() % 100 < 45) m->grid[y][x] = TILE_PELLET;
        } else if (tile == TILE_PELLET || tile == TILE_EMPTY) {
            if (rand() % 100 < 20) m->grid[y][x] = TILE_WALL;
        }
    }
    ...
    for (int y = 2; y < MAP_H - 2; y++) {
        for (int x = 2; x < MAP_W - 2; x++) {
            if (model_is_special_cell(x, y, variant)) continue;
            if (m->grid[y][x] == TILE_EMPTY && rand() % 100 < 10) {
                m->grid[y][x] = TILE_PELLET;
            }
        }
    }
}
...
void model_load_map(GameModel *m) {
    ...
    m->map_selected = variant;
    model_apply_map_variant(m, variant);
    if (m->level >= 4) {
        model_apply_randomized_layout(m, variant);
    }
    model_recount_pellets(m);

    m->frame_ms = model_compute_frame_ms(m);
    ...
    m->ghost_eat_combo = 0;
    m->frame_count = 0;
    m->phase_ghosts_eaten = 0;
    m->phase_fruits_eaten = 0;
    m->phase_start_ticks = 0;
    /* Ready message: ~2000ms rounded to frames based on frame_ms */
    m->ready_timer = (m->frame_ms > 0) ? (2000 / m->frame_ms) : 25;
...
void model_reset_positions(GameModel *m) {
    ...
    m->power_timer = 0;
    m->ghost_eat_combo = 0;
    /* Ready message after death: ~2000ms */
    m->ready_timer = (m->frame_ms > 0) ? (2000 / m->frame_ms) : 25;
}
