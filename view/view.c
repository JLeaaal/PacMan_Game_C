...
static void restore_map_region(GameModel *m, int col, int row, int w, int h) {
    int first_cell_x = (col - OFFSET_X) / CELL_W;
    int last_cell_x = (col + w - 1 - OFFSET_X) / CELL_W;
    int first_cell_y = row - OFFSET_Y;
    int last_cell_y = row + h - 1 - OFFSET_Y;

    if (first_cell_x < 0) first_cell_x = 0;
    if (last_cell_x >= MAP_W) last_cell_x = MAP_W - 1;
    if (first_cell_y < 0) first_cell_y = 0;
    if (last_cell_y >= MAP_H) last_cell_y = MAP_H - 1;

    for (int y = first_cell_y; y <= last_cell_y; y++) {
        for (int x = first_cell_x; x <= last_cell_x; x++) {
...
static void panel_text(int row, const char *color, const char *text) {
    ...
    printf("%s%s%s", color, buf, CLR_RESET);
...
void view_draw_side_panel(GameModel *m) {
    ...
    {
        panel_header(r++, "UPS / FASE");
        char buf[80];

        snprintf(buf, sizeof(buf), "1) Fant:%d", m->cumulative_ghosts_eaten + m->phase_ghosts_eaten);
        panel_text(r++, CLR_ORANGE, buf);

        snprintf(buf, sizeof(buf), "2) B:%d P:%d",
                 m->cumulative_ghosts_eaten_by_ghost[0] + m->ghosts[0].eaten,
                 m->cumulative_ghosts_eaten_by_ghost[1] + m->ghosts[1].eaten);
        panel_text(r++, CLR_PINKY, buf);
        snprintf(buf, sizeof(buf), "   I:%d C:%d",
                 m->cumulative_ghosts_eaten_by_ghost[2] + m->ghosts[2].eaten,
                 m->cumulative_ghosts_eaten_by_ghost[3] + m->ghosts[3].eaten);
        panel_text(r++, CLR_CLYDE, buf);

        snprintf(buf, sizeof(buf), "3) Frut:%d", m->cumulative_fruits_eaten + m->phase_fruits_eaten);
        panel_text(r++, CLR_GREEN, buf);

        int elapsed = 0;
        if (m->phase_start_ticks > 0) {
            elapsed = (int)(clock() / CLOCKS_PER_SEC) - m->phase_start_ticks;
            if (elapsed < 0) elapsed = 0;
        }
        snprintf(buf, sizeof(buf), "4) T:%d s", elapsed);
        panel_text(r++, CLR_CYAN, buf);

        panel_text(r++, CLR_DIM, "");
    }
    panel_sep(r++);
...
void view_clear_ready(GameModel *m) {
    int col = OFFSET_X + (MAP_W * CELL_W / 2) - 6;
    int row = OFFSET_Y + 17;
    restore_map_region(m, col, row, 14, 3);
}
...
void view_restore_bonus_cell(GameModel *m, int x, int y) {
    ...
    restore_map_region(m, sx, sy, 8, 1);
}
...
void view_draw_bonus(GameModel *m) {
    ...
    clear_rect(sx, sy, 8, 1);
    move_cursor(sx, sy);
    const char *col = (m->bonus_points >= 800)  ? CLR_DIM :
                      (m->bonus_points >= 400)  ? CLR_DIM2 :
                      (m->bonus_points >= 200)  ? CLR_DIM :
                                                  CLR_DIM2;
...
void view_draw_splash(GameModel *m) {
    ...
    if (blink) {
        const char *msg = "PRESSIONE ENTER PARA COMEÇAR";
...
void view_draw_menu(GameModel *m) {
    ...
    cy += 7;
    if (!menu_drawn) {
        const char *sub = "Julia Leal · Estruturas de Dados · UniCesumar";
...
void view_draw_skin_selector(GameModel *m) {
    ...
    print_abs(cx + 10, cy + 3, CLR_DIM, "    ESC para voltar ao menu");
...
void view_draw_win(GameModel *m) {
    ...
    printf("%sFASE %d CONCLUIDA%s", CLR_WIN_CLR, m->level, CLR_RESET);
    move_cursor(cx + 4, cy + 5);
    printf("%sSCORE DA FASE%s   %s%d%s    %sPRÓXIMO NIVEL%s   %s%d%s",
           CLR_DIM, CLR_RESET, CLR_TITLE, m->score, CLR_RESET,
           CLR_DIM, CLR_RESET, CLR_CYAN, m->level + 1, CLR_RESET);

    /* Estatísticas da fase */
    move_cursor(cx + 4, cy + 7);
    printf("%sESTATÍSTICAS DA FASE%s", CLR_GREEN, CLR_RESET);
    ...
    move_cursor(cx + 4, cy + 9);
    printf("%s1. Fantasmas comidos na fase: %d%s", CLR_CYAN, m->phase_ghosts_eaten, CLR_RESET);
    move_cursor(cx + 4, cy + 10);
    printf("%s2. Blinky: %d   Pinky: %d   Inky: %d   Clyde: %d%s",
           CLR_PINKY, m->ghosts[0].eaten, m->ghosts[1].eaten, m->ghosts[2].eaten, m->ghosts[3].eaten, CLR_RESET);
    move_cursor(cx + 4, cy + 11);
    printf("%s4. Frutinhas pequenas: %d%s", CLR_GREEN, m->phase_fruits_eaten, CLR_RESET);
    move_cursor(cx + 4, cy + 12);
    printf("%s5. Tempo decorrido: %d s%s", CLR_CYAN, elapsed, CLR_RESET);
